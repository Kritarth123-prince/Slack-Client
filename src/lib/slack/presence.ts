...
async function syncPresenceToSlack(userId: string, presence: "active" | "away"): Promise<void> {
  try {
    const client = await getSlackClientForUser(userId);
    await client.users.setPresence({ presence: presence === "away" ? "away" : "auto" });
  } catch (err) {
    if (!isMissingScope(err)) logger.error("Failed to sync presence to Slack", { message: (err as Error).message });
  }
}
...
async function syncCustomStatusToSlack(
  ...
  try {
    ...
    if (!isMissingScope(err)) logger.error("Failed to sync custom status to Slack", { message: (err as Error).message });
  }
}
...
async function syncDndToSlack(userId: string, enabled: boolean): Promise<void> {
  try {
    const client = await getSlackClientForUser(userId);
    if (enabled) await client.dnd.setSnooze({ num_minutes: 24 * 60 });
    else await client.dnd.endSnooze();
  } catch (err) {
    if (!isMissingScope(err)) logger.error("Failed to sync Do Not Disturb to Slack", { message: (err as Error).message });
  }
}

function toView(pref: {
  presence: string;
  presenceManual: boolean;
  autoAwayEnabled: boolean;
  doNotDisturb: boolean;
...
export async function getStatus(userId: string): Promise<StatusView> {
  ...
  if (Object.keys(data).length > 0) {
    ...
    if (data.presence) await syncPresenceToSlack(userId, data.presence as "active" | "away");
    if ("statusEmoji" in data) await syncCustomStatusToSlack(userId, null, null, null);
  }

  return toView({ ...pref, presence, statusEmoji, statusText, statusExpiresAt });
}
...
export async function updateStatus(userId: string, input: UpdateStatusInput): Promise<StatusView> {
  ...
  if (data.presence) await syncPresenceToSlack(userId, pref.presence as "active" | "away");
  if (typeof input.doNotDisturb === "boolean") await syncDndToSlack(userId, input.doNotDisturb);
  if (input.clearStatus || input.statusEmoji !== undefined || input.statusText !== undefined || input.statusExpiresAt !== undefined) {
    await syncCustomStatusToSlack(userId, pref.statusEmoji, pref.statusText, pref.statusExpiresAt);
  }
